...
public class VentaDAO {
	...
	public VentaVO[] getDatos(){
		...
		if(conex.conectado()){
			try{
				...
				PreparedStatement consulta = conex.getConnection().prepareStatement("SELECT id, cliente, importe, vendedor, fecha FROM ventas");
	...
	public VentaVO lastInsert(){		
		...
		if(conex.conectado()){
			try{
			PreparedStatement consulta = conex.getConnection().prepareStatement("SELECT id, cliente, importe, vendedor, fecha FROM ventas WHERE `id` = ?");
